...
public class AuthenticationRestControllerTest extends BudgeezBootApplicationTests {

    private String name = "Test";
    private String email1 = "test1@budgeez.com";
    private String email2 = "test2@budgeez.com";
    private String email3 = "test3@budgeez.com";
    ...
    public void testGetUserDetailsInvalidToken() {
        HttpResponseJson response = authenticationRestControllerConnectorHelper.getUserDetailsNegative(token.substring(1, token.length()-1)).convertToHttpResponseJson();
        assertEquals(response.getHttpStatusCode(), 401);
        assertEquals(response.getObject().get("error").getAsString(), "Unauthorized");
        assertEquals(response.getObject().get("message").getAsString(), "Unauthorized");
    }
    ...
    public void testIsEmailRegisteredTrue() {
        String response = (String) authenticationRestControllerConnectorHelper.isEmailRegistered(email1).getObject();
        assertTrue(Boolean.parseBoolean(response));
    }

    @Test
    public void testIsEmailRegisteredFalse() {
        String response = (String) authenticationRestControllerConnectorHelper.isEmailRegistered("abc@def.co").getObject();
        assertFalse(Boolean.parseBoolean(response));
    }

    @Test
    public void testChangeEmailSendRequestValid() {
        ChangeEmailWrapper wrapper = new ChangeEmailWrapper();
        wrapper.setEmail("newemail@domain.ru");
        wrapper.setPassword(password);
        HttpResponseJson response = authenticationRestControllerConnectorHelper.changeEmailNegative(wrapper, token).convertToHttpResponseJson();
